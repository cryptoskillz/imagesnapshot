...
export async function onRequestPut(context) {
    ...
    //check its valid
    if (token == "") {
        return new Response(JSON.stringify({ error: "not authorised to update templates" }), { status: 400 });
    } else {
        ...
        if (contentType != null) {
            ...
            //const encodedHtmlCode = encodeURIComponent(theData.template);
            const KV = context.env.backpage;
            await KV.put(`template-${theData.projectId}`, theData.template);
            //build the query
            let theQuery = `UPDATE projects SET templateName = '${theData.templateName}' where id = '${theData.projectId}'`
